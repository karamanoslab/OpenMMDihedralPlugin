...
class Sim:
    ...
    def build_system(self):
        ...
        * set restraints
        ...
        try:
            self.comp_defaults['use_dih']
        except KeyError:
            self.comp_defaults['use_dih']=False
        
        if self.comp_defaults['use_dih']:
            res_dict={}
            self.timestep=0.005 #picoseconds
            for comp in self.components:
                
                try:
                    self.residues_epsd = read_csv(comp.fresidues_epsds).set_index('three')
                except AttributeError:
                    raise FileNotFoundError(f'Residue parameter file name (fresidues_epsds) not supplied to component {name}.')
                
                for r, res in enumerate(comp.seq):            
                    resname = str(seq3(res)).upper()
                    res_dict[r]=resname
            
                try:
                    dihdomains=comp.calc_dihdomains()
                except AttributeError:
                    raise FileNotFoundError(f'Residue parameter file name (dih_domains) not supplied to component {name}.')
                except KeyError: 
                    nParticles=len(comp.seq)
                    dihdomains=[0, nParticles-1]
                    
                print('DIH restraints for residues:', dihdomains)
                for domain in dihdomains:
                    self.dih=interactions.init_dih_plugin(res_dict, self.residues_epsd['epsds'], torsions=[ domain[0]+1, domain[-1]+1 ])
                    self.ang=interactions.init_angle_potential( angles=[ domain[0]+1, domain[-1]+1 ])
        else:
            self.timestep=0.01
    ...
    def add_forces_to_system(self):
        ...
        #dih forces
        if self.comp_defaults['use_dih']:
            for force in [self.dih, self.ang]:
                self.system.addForce(force)
...
#             self.add_restraints(comp,offset)
    ...
    def get_energies(self, sim):
        for i in range(self.system.getNumForces()): #[0, 1, 2, 3]:
    ...
    def simulate(self):
        ...
        integrator = openmm.openmm.LangevinMiddleIntegrator(self.temp*unit.kelvin,self.friction_coeff/unit.picosecond,self.timestep*unit.picosecond)
        ...
        if self.platform == 'CPU':
            ...
            if self.comp_defaults['use_dih']:
                raise Exception('DIH restraints only support the CPU platform')
        ...
        if self.slab_eq:
            ...
            for index, force in enumerate(self.system.getForces()):
                if isinstance(force, openmm.CustomExternalForce):
                    ...
                    break
            integrator = openmm.openmm.LangevinIntegrator(self.temp*unit.kelvin,self.friction_coeff/unit.picosecond,self.timestep*unit.picosecond)
            if self.platform == 'CPU':
                simulation = app.simulation.Simulation(pdb.topology, self.system, integrator, platform, dict(Threads=str(self.threads)))
            else:
                simulation = app.simulation.Simulation(pdb.topology, self.system, integrator, platform)
