...
def init_dih_plugin(res_dict, epsd_dict, torsions=[]):

    import dihedralplugin
        
    force = dihedralplugin.DihedralForce()
    print('Initialising dihedral potential')
    for i in range(torsions[0], torsions[1]-3, 1):
        torsion=np.arange(i, i+4, 1)
        epsd=0.5* (epsd_dict[res_dict[torsion[1]]] + epsd_dict[res_dict[torsion[2]]])

        print(torsion, i, epsd, [res_dict[res]  for res in torsion])
   
        force.addDihedral(int(torsion[0]), int(torsion[1]), int(torsion[2]), int(torsion[3]),
			   11.4, 0.9, epsd, 0.15, 1.02, #double ka1, double phia1, double epsd, double ka2, double phia2
...
def init_angle_potential( angles=[]):
    g=0.1
    ka=106.4
    theta_a=1.6
    kb=26.3
    theta_b=2.27
    energy_expr=f"exp(-{g}*{ka}*(theta - {theta_a})^2 + epsa) + exp(-{g}*{kb}*(theta - {theta_b})^2) "
    ang=openmm.CustomAngleForce(f"-log({energy_expr})/{g}")

    ang.addPerAngleParameter('epsa')
    epsa=4.3

    for i in range(angles[0], angles[1]-2,1 ):
        a=np.arange(i, i+3, 1)
        ang.addAngle(a[0], a[1], a[2], [epsa])
	
    ang.setUsesPeriodicBoundaryConditions(True)
    ang.setForceGroup(11)
    return ang
