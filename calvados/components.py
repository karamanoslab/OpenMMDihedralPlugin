...
class Component:
    ...
    def __init__(self, name: str, properties: dict, defaults: dict):
        ...
        # if defaults['use_dih']:
#             try:
#                 self.residues_epsd = read_csv(self.fresidues_epsds).set_index('one')
#             except AttributeError:
#                 raise FileNotFoundError(f'Residue parameter file name (fresidues_epsds) not supplied to component {name}.')
    ...
    def calc_comp_seq(self):
        ...
        if self.restraint:
            ...
            #self.seq = str(records[self.name].seq) #override from fasta to allow for easy mutatoins in fasta file
        else:
...
class Protein(Component):
    ...
    def calc_dihdomains(self):
        """ Get bounds for restraints (dih). """

        self.dihdomains = build.get_ssdomains(self.name,self.dihdomains)
#         print("dih domains", self.dihdomains)
        return  self.dihdomains
    
    def calc_ssdomains(self):
        """ Get bounds for restraints (harmoinc). """
